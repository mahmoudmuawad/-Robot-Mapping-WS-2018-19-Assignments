...
% The current pose estimate of the landmark with id = j is: [mu(2*j+2); mu(2*j+3)]  --> like arithmatic series
...
for i = 1:m
	...
	Z(1+2*i - 2, 1) = z(i).range;
  Z(2*i, 1) = z(i).bearing;
  
	% TODO: Use the current estimate of the landmark pose
	% to compute the corresponding expected measurement in expectedZ:
  deltax = mu(2*landmarkId+2) - mu(1);
  deltay = mu(2*landmarkId+3) - mu(2);
  delta = [deltax;deltay];
  q = transpose(delta)*delta;
  
  expectedZ(1+2*i -2) = sqrt(q);
  expectedZ(2*i) = normalize_angle(atan2(deltay,deltax) - mu(3)); 
  


	% TODO: Compute the Jacobian Hi of the measurement function h for this observation
  Hi = 1/q *[-sqrt(q)*deltax,-sqrt(q)*deltay,0,sqrt(q)*deltax,sqrt(q)*deltay; deltay,-deltax,-q,-deltay,deltax];
...
mu += K*normalize_all_bearings(Z-expectedZ);
